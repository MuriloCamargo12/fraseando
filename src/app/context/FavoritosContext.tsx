...
export default function FavoritosProvider({ children }: { children: ReactNode }) {
    ...
    function adicionarFavorito(item: Favorito) {
        setFavoritos((prev) => {
            ...
            if (item.frase === 'Limite atingido: você pode gerar até 5 frases a cada' || item.frase === "Não foi possível traduzir a frase" || item.frase === "Você já pode gerar uma nova frase.") return prev
